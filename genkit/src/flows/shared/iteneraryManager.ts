...
import { getActivitiesForPlace } from '@sudoku/backend';
...
const exchangePhotoUri = (photoUri: string): Promise<string> => {
    ...
    axios.get(exchangeUrl)
        ...
        .catch((e) => {
            ...
            resolve("https://storage.googleapis.com/sudoku-imgs/default/dinerDefault.png");
        })
    });
    return url;
}
...
const formatPhoto = async (activityRef: string, locationRef: string, photoUri?: string): Promise<string> => {
    ...
    return `https://storage.googleapis.com/sudoku-imgs/activities/${locationRef}_${activityRef}.jpg`;
}
